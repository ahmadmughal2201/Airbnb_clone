...
const Type = ({collection}) => {
  ...
  const {money, setMoney, roomId, setRoomId, type, setType} = useMyContext();
  ...
  const getAllRooms = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3000/api/get-rooms-type/${type}`);
      setRooms(data.rooms);
    } catch (error) {
      saveLogs(error.message, "/room-type", "User");
